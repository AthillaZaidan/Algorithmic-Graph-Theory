...
json hitungJumlahIsland(const vector<string>& grid) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            if ((grid[i][j] == '1' || grid[i][j] == '*') && labels[i][j] == 0) {
                islandCount++;
                ...
                while (!q.empty()) {
                    ...
                    for (auto [dr,dc] : vector<pair<int,int>>{{-1,0},{1,0},{0,-1},{0,1}}) {
                        int nr = r+dr, nc = c+dc;
                        if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && (grid[nr][nc] == '1' || grid[nr][nc] == '*') && labels[nr][nc] == 0) {
...
json solveTimetabling(int teacherCount, int classCount, const vector<vector<int>>& requirements, int roomLimit) {
    ...
    for (int i = 0; i < teacherCount; i++) {
        for (int j = 0; j < classCount; j++) {
            ...
            teacherLoads[i] += need;
            classLoads[j] += need;
            for (int r = 0; r < need; r++) edges.push_back({i, j, -1, (int)edges.size()});
            totalLessons += need;
        }
    }
    ...
    if (roomLimit > 0 && totalLessons > 0) periodCount = max(delta, (totalLessons + roomLimit - 1) / roomLimit);

    if (totalLessons == 0) {
        json response = {{"success", true}, {"periodCount", 0}, {"delta", delta}, {"totalLessons", totalLessons}, {"teacherLoads", teacherLoads}, {"classLoads", classLoads}, {"assignments", json::array()}, {"periodSizes", json::array()}};
    ...
    auto firstFreeTeacher = [&](int teacher) {
        for (int c = 0; c < periodCount; c++) if (teacherColor[teacher][c] == -1) return c;
        return -1;
    };
    auto firstFreeClass = [&](int classId) {
        for (int c = 0; c < periodCount; c++) if (classColor[classId][c] == -1) return c;
        return -1;
    };
    ...
    for (int edgeId = 0; edgeId < (int)edges.size(); edgeId++) {
        ...
        if (alpha == -1 || beta == -1) return {{"success", false}, {"error", "Tidak ada warna bebas untuk alternating path"}};
    ...
    auto recomputePeriodSizes = [&]() {
        ...
        for (const auto& e : edges) if (e.color >= 0) periodSizes[e.color]++;
    };
    ...
    auto rebalancePair = [&](int highColor, int lowColor) {
        ...
        for (const auto& startEdge : edges) {
            ...
            if (highCount > lowCount) {
                for (int edgeId : componentEdges) edges[edgeId].color = edges[edgeId].color == highColor ? lowColor : highColor;
                recomputePeriodSizes();
                return true;
            }
        }
    ...
    if (roomLimit > 0) {
        ...
        while (guard-- > 0) {
            ...
            if (!rebalancePair(highColor, lowColor)) return {{"success", false}, {"error", "Balancing Lemma 6.3 gagal menemukan komponen penukar"}};
        }
    }
    ...
    for (int edgeId : order) {
        ...
        assignments.push_back({{"teacher", e.teacher}, {"class", e.classId}, {"period", e.color}, {"edgeId", e.id}});
    }

    json response = {{"success", true}, {"periodCount", periodCount}, {"delta", delta}, {"totalLessons", totalLessons}, {"teacherLoads", teacherLoads}, {"classLoads", classLoads}, {"assignments", assignments}, {"periodSizes", periodSizes}};
    if (roomLimit > 0) response["roomLimit"] = roomLimit;
    return response;
}
    ...
    try {
        ...
        if (operation == "dfs" || operation == "bfs" || operation == "check_path" || operation == "check_connectivity") {
            ...
            if (teacherCount <= 0 || classCount <= 0 || teacherCount + classCount > 1024) {
                resultBuffer = json{{"success", false}, {"error", "Jumlah guru + kelas harus 1-1024"}}.dump();
            ...
            if (!input.contains("requirements") || !input["requirements"].is_array() || (int)input["requirements"].size() != teacherCount) {
                resultBuffer = json{{"success", false}, {"error", "Matrix requirements tidak sesuai jumlah guru"}}.dump();
                return resultBuffer.c_str();
            }
            ...
            for (int i = 0; i < teacherCount; i++) {
                if (!input["requirements"][i].is_array() || (int)input["requirements"][i].size() != classCount) {
                    resultBuffer = json{{"success", false}, {"error", "Matrix requirements tidak sesuai jumlah kelas"}}.dump();
                    return resultBuffer.c_str();
                }
                for (int j = 0; j < classCount; j++) {
                    int value = input["requirements"][i][j].get<int>();
                    if (value < 0) {
                        resultBuffer = json{{"success", false}, {"error", "Nilai p_ij tidak boleh negatif"}}.dump();
                        return resultBuffer.c_str();
                    }
            ...
            if (totalLessons > 100000) {
                resultBuffer = json{{"success", false}, {"error", "Total kebutuhan mengajar terlalu besar (max 100000)"}}.dump();
                return resultBuffer.c_str();
            }
            int roomLimit = input.value("roomLimit", 0);
            if (roomLimit < 0) {
                resultBuffer = json{{"success", false}, {"error", "Kapasitas ruangan tidak boleh negatif"}}.dump();
                return resultBuffer.c_str();
            }
